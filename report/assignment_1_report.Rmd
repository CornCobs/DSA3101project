...
```{r echo=F}
cat_info %>% head() %>% 
  kable(caption = "Figure 1: First 6 rows of DSA3101_Hackathon_Categories_Information.csv")
...
select_if(cat_info, is.numeric) %>% summary() %>% 
  kable(caption = "Figure 2: Summary Statistics")
```
...
```{r echo=F}
data %>% head() %>% 
  kable(caption = "Figure 3: First 6 rows of DSA3101_Hackathon_Data.csv")
```
...
select_if(data, is.numeric) %>% summary() %>% 
  kable(caption = "Figure 4: Summary Statistics")
```
...
```{r echo=F, message=F}
panel_demo %>% head() %>% 
  kable(caption = "Figure 5: First 6 rows of DSA3101_Hackathon_Panelists_Demographics.csv")
```

```{r echo=F, message=F}
lapply(colnames(panel_demo)[2:8], 
       function(x){table(panel_demo[[x]])/nrow(panel_demo)}) %>% 
  kable(col.names=c("", "Proportion"), caption = "Figure 6: Proportion of each columns")
```
...
panel_demo %>% 
  ...
  mutate(n=n/sum(n)) %>%
  spread(key=Income, value=n) %>% kable(caption = "Figure 7: Location VS Income")
```
...
head(filter(data, `Pack Size` == 0 | `Volume` == 0 | `Spend` == 0)) %>% 
  kable(caption = "Figure 9: Examples of erroneous data")
```
...
py$data_cleaned %>% select(4:6) %>% summary() %>% 
  kable(caption = "Figure 10: Summary Statistics after imputation")
```
...
data %>% filter(Volume == 0, `Panel ID` == "Panel 108052110") %>% 
  head(2) %>% kable(caption = "Figure 11: Examples of preimputed errorneous data")
py$example_cleaned %>% mutate(Date=as_date(Date)) %>% head(2) %>% 
  kable(caption = "Figure 12: Examples of postimputed errorneous data")
...
relative %>% group_by(`Panel ID`) %>% 
  ...
  kable(caption="Figure 14: Relative changes in RFM between Pre MCO and Post MCO")
```
...
preMCO %>% filter(Recency == 3, Frequency == 3, Monetary == 1) %>% 
  ...
  kable(caption="Figure 15: Income subgroups of RFM = 331 before MCO")
```
...
lapply(colnames(preMCO)[c(7, 10:14)], 
  ...
  kable(col.names=c("", "Proportion"), 
        caption="Figure 17: Proportion of each columns for Income > 5000 with RFM = 331 before MCO")
```
...
lapply(colnames(preMCO)[c(7, 10:14)], 
       function(x){table(demo_als[[x]])/nrow(demo_als)}) %>% 
  kable(col.names=c("", "Proportion"), 
        caption="Figure 18: Proportion of each columns for Income > 5000 with RFM = 331 after MCO")
```
...
lapply(colnames(m_inc)[c(3,4,6:10)], 
  ...
  kable(col.names=c("", "Proportion"),
        caption="Figure 19: Proportion of each columns for customers with R = 3, F = 3 and experienced an increase in M after MCO")
```
